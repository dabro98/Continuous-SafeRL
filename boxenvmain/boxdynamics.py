...
#CFG_PATH = "boxenvmain/config_2.json"
CFG_PATH = "boxenvmain/config.json"

#CFG_PATH = "config_2.json"
...
class AgentCfg:
    ...
    in_safety_critical_area: bool = False
...
class BoxEnv(gym.Env):
    ...
    def resetAgent(self):
        ...
        self.agent_body.position = agent_pos

        self.agent_body.angle = agent_angle

        self.agent_body.linearVelocity = b2Vec2(1, 1)
        logging.info("resetted agent")
    ...
    def step(self, action) -> tuple:
        ...
        # self.get_critical_area_reward()

        # current and previous state
        self.state = self.get_observations()
        assert self.state in self.observation_space

        # check whether the current step violates safety
        violation = self.check_current_safety_violation()

        # agent rewards
        step_reward = self.set_reward()
        self.total_reward += step_reward

        new_violation = violation and not self.cfg.agent.in_safety_critical_area

        self.cfg.agent.in_safety_critical_area = violation

        # TODO: info to return
        info = {"violation": new_violation}
        ...
        self.step_counter += 1
        if self.step_counter % self.reset_n_steps == 0:
            self.reset()
    ...
    def get_critical_area_reward(self):
        for body in self.cfg.design_bodies:
            if body.physic["safety"] == 1:
                print(f"critical area reward: {body.physic['reward']}")
                return body.physic["reward"]

    def set_reset_n_steps(self, n):
        self.reset_n_steps = n
        print("SET reset_n_steps = ", n)
    ...
    def set_reward(self):
        ...
        for agent_contacts in self.agent_body.userData.contacts:
            if self.cfg.agent.in_safety_critical_area:
                continue
            step_reward += agent_contacts.userData.reward
        return step_reward

    def check_current_safety_violation(self) -> bool:
        current_step_violating_safety = False
        for agent_contacts in self.agent_body.userData.contacts:
            if agent_contacts.userData.safety == 1:
                current_step_violating_safety = True
        return current_step_violating_safety
    ...
    def world_and_load_design_without_user_input(self, env_path):
        # getting bodies already inside config file
        for design in copy_design_bodies(self.cfg.design_bodies):
            self.ui.design_bodies.append(design)

        self.ui.set_mode(UIMode.RESIZE)
        self.load_design(env_path)
        self.ui.set_mode(UIMode.SIMULATION)
    ...
    def create_body(self, design_data: DesignData):
        ...
        body.userData.safety = design_data.physic["safety"]
    ...
    def get_observation_dict(self):
        ...
        for key in self.cfg.agent.obs_keys:
            if key == "distances":
                ...
                partial_dict = ({"reward": gym.spaces.Box(
                    low=-100, high=np.inf, shape=(self.cfg.agent.obs_num,))})
    ...
    def set_observation_dict(self):
        ...
        for key in self.cfg.agent.obs_keys:
            if key == "distances":
                ...
                state["reward"] = np.array(
                    [observation.body.userData.reward for observation in self.data], dtype=np.float32)
            # elif key == "body_types":
